    <section class="feature-four">
        <div class="container" dir="rtl">
            <div class="row">
                <!-- Feature Four Single Start -->
                <div class="col-xl-4 col-lg-4">
                    <div class="feature-four__single">
                        <div class="feature-four__img-box">
                            <div class="feature-four__img">
                                <img src="assets/images/resources/feature-4-1.jpg" alt="">
                            </div>
                            <div class="feature-four__icon">
                                <span class="icon-buildings"></span>
                            </div>
                        </div>
                        <h3 class="feature-four__title"><a href="our-services.html">استطلاعات الرأي</a></h3>
                        <!-- Add Button under h3 -->
                        <br>
                        <a href="link-to-another-page-1.html" class="btn btn-primary">المزيد</a>
                    </div>
                </div>
                <!-- Feature Four Single End -->

                <!-- Feature Four Single Start -->
                <div class="col-xl-4 col-lg-4">
                    <div class="feature-four__single">
                        <div class="feature-four__img-box">
                            <div class="feature-four__img">
                                <img src="assets/images/resources/feature-4-2.jpg" alt="">
                            </div>
                            <div class="feature-four__icon">
                                <span class="icon-goals"></span>
                            </div>
                        </div>
                        <h3 class="feature-four__title"><a href="our-services.html">افكار و مبادرات</a></h3>
                        <!-- Add Button under h3 -->
                        <br>
                        <a href="link-to-another-page-2.html" class="btn btn-primary">المزيد</a>
                    </div>
                </div>
                <!-- Feature Four Single End -->

                <!-- Feature Four Single Start -->
                <div class="col-xl-4 col-lg-4">
                    <div class="feature-four__single">
                        <div class="feature-four__img-box">
                            <div class="feature-four__img">
                                <img src="assets/images/resources/feature-4-3.jpg" alt="">
                            </div>
                            <div class="feature-four__icon">
                                <span class="icon-ferris"></span>
                            </div>
                        </div>
                        <h3 class="feature-four__title"><a href="our-services.html">شكاوي و اقتراحات</a></h3>
                        <!-- Add Button under h3 -->
                        <br>
                        <a href="link-to-another-page-3.html" class="btn btn-primary">المزيد</a>
                    </div>
                </div>
                <!-- Feature Four Single End -->

            </div>
        </div>
    </section>